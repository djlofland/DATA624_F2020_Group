...
Note - we are doing an observational study so any correlations we identify would need to be followed up with testing to identify causal relationships.  Since ABC beverage We apply machine learning approaches to historical company data to build then select a predictive model of pH. This model could help the company adapt its processes in a changing regulatory environment.
...
Based on evaluation both RMSE and $R^2$, our Gradient Boosted Model (GBM) out performed the other models.  This is not surprising as GBM models are more tolerant of multicollinearity and non-linear features.  
