...
One problem that can occur with multiple regression is a correlation between predictors, or multicollinearity.  A quick check is to run correlations between all model features.   
...
We can see that the `Balling`- and `Carb`-related features are highly correlated within groups, with positive correlations between 0.75 and 1. In general, this dataset is challenging as many of the predictive features go hand-in-hand with others, and our models will need to account for this multicollinearity.
