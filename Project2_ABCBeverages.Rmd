...
Our team's analysis seeks to build understanding of the ABC Beverage manufacturing process and the related factors that affect the pH of the company's beverage products. We apply machine learning approaches to historical company data to build then select a predictive model of pH. This model could help the company adapt its processes in a changing regulatory environment.
...
As expected, the dummy variables, e.g. ` ``Brand Code``A`, appear binary. We still have bimodal features as we did not apply any feature engineering on them. A few features, including `PSC Fill` and `Temperature`, still show skew, but they seem closer to normal. Our transformations are complete, and we can continue on to building our models.
...
vip(model1, color = "red", fill = "purple")
...
A gradient boosting machine approach, or GBM, encompasses both regression and classification. GBM uses a loss function and a weak learner to build an additive model that minimizes that loss function.
...
results <- data.frame(t(postResample(pred = gbmPred, obs = df_test$PH))) %>% mutate(Model = "Generalized Boosted Regression Modeling (GBM)") %>% rbind(results)
