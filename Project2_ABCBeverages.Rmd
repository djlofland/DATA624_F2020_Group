...
With a now solid understanding of our dataset, and with our data cleaned, we can now start to build out candidate models. First, we split our cleaned dataset into training and testing sets (80% training, 20% testing). This split is necessary as the provided evaluation data set does not provide `PH` values, meaning we cannot measure our model performance against that dataset. 
