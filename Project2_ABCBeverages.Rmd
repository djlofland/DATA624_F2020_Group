...
results <- data.frame(t(postResample(pred = model1_pred, obs = df_test$PH))) %>% 
    mutate(Model = "Mutiple Regression") %>% rbind(results)
```

*INSERT discussion here*

#### Model \#2 *(Let's skip Partial Least Squares (PLS) model here. Ridge Regression)*
...
#### Model \#6 Generalized Boosted Regression Modeling (GBM). Dump XGBoost
...
#### Model \#7 *(Kera Deep Learning NN). This might be a redundant method to Model \#4. I chose MARS to replace the original choosing of Kera Neural Network in Kera.* Multivariate Adaptive Regression Splines (MARS)
